...
clean_non_48 <- function(output_data)
{
  ...
  output_data$geo_county[output_data$geo_county == "Saint Thomas County"] <- "Saint Thomas Island County"
...
clean_county_names <- function(cset, sset) {
  ...
  cset[sset == "Nevada" & cset == "Ormsby County"] <- "Ormsby (Ext) County" # 3
  ...
  cset[sset == "Virginia" & cset == "Hampton County"] <- "Hampton (Ic) County" # 1
  cset[sset == "Virginia" & cset == "Loudon County"] <- "Loudoun County" # 3
  cset[sset == "Virginia" & cset == "Portsmouth County"] <- "Portsmouth (Ic) County" # 3
