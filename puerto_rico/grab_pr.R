...
if (!file.exists(id_path <- file.path("web", "ids.txt")))
{
  href_list <- c()
  i_list <- c()
  for (i in seq_len(83))
  {
    url <- sprintf("https://www.loc.gov/pictures/search/?q=puerto+rico&sp=%d&co=fsa", i)
    x <- read_html(url)
  ...
  table(table(i_list)) # all 20, except the last ?
